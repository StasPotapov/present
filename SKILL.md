...
во внешнем редакторе — и подхватываются на лету; на самой деке их нет. Рядом с декой живёт `DECK.md` — манифест (карта слайдов, решения, как править).
...
скрипт — сохрани его **в папку деки** (не в scratchpad!) вместе с исходниками, из которых он собирает,
и держи актуальным после любых правок (пересборка не должна откатывать деку); режим зафиксируй в
`DECK.md` (Шаг 5).
...
4. **Медиа** (умный дефолт): лёгкое (< ~800 КБ) → inline base64 (дека остаётся одним офлайн-файлом);
   тяжёлое (> ~800 КБ, крупные GIF/видео) → sidecar-папка `deck-assets/` рядом + **предупреди**, что дека больше не автономна.
5. **Сгенерируй файл живых заметок** `<slug>-notes.js` рядом с декой из data-notes (имя = имя html-файла
   деки + `-notes.js` — kernel ищет его именно так):
   ```js
...
- **Сгенерируй `DECK.md`** рядом с декой по `assets/deck-template.md`: режим деки (скрипт/руками), карта
  слайдов, арт-дирекшн сжато, принятые решения/отступления с «почему», как править. Собирал скриптом —
  проверь, что скрипт и его исходники лежат в папке деки.
- Сохрани `<slug>-<дата>.html` рядом с исходником (или спроси путь). Открой: `open <файл>` (macOS).
- Напомни управление: `← →`/колесо/свайп — листать, `P` — presenter-окно с заметками (его НЕ шарить, шарить деку),
  ...
  **Правка заметок прямо в presenter-окне — кнопками мышью** (буквенных хоткеев там намеренно нет: в это окно
  ...
  Пересылать деку можно одним HTML (вшитые заметки-fallback), но выступать — из папки, рядом с notes.js.
...
`<slug>-notes.js` (он источник правды для заметок; `data-notes` в HTML обнови как fallback). После правок
обнови `DECK.md` (карту слайдов и раздел решений), если менял структуру/анимации/тайминги.
